...
def BuildName(fname):
    ...
    buf = [s.capitalize() for _, s in enumerate(name.split("_"))]
    return "Type" + "".join(buf)

def BuildBody(parsers):
    def Parse(parser):
        if parser.mParse == ParseBool:
            return parser.mName != None and "%s: boolean" % parser.mName or "boolean"
        elif parser.mParse == ParseFloat:
            return parser.mName != None and "%s: number" % parser.mName or "number"
        elif parser.mParse == ParseString:
            return parser.mName != None and "%s: string" % parser.mName or "string"
        elif parser.mParse == ParseType:
            buf = []
            for _, v in enumerate(parser.mChild):
                buf.append(Parse(v))
            return parser.mName != None and "%s: {%s}" % (parser.mName, ", ".join(buf)) or "{%s}" % ", ".join(buf)
        elif parser.mParse == ParseList:
            return parser.mName != None and "%s: %s[]" % (parser.mName, Parse(parser.mChild[0])) or "%s[]" % Parse(parser.mChild[0])
        elif parser.mParse == ParseDict:
            return parser.mName != None and "%s: {[key: string]: %s}" % (parser.mName, Parse(parser.mChild[1])) or "{[key: string]: %s}" % Parse(parser.mChild[1])
        assert(False)

    assert(parsers[0].mParse == ParseFloat or \
           parsers[0].mParse == ParseString)
    buf = []
    for _, parser in enumerate(parsers):
        buf.append(Parse(parser))
    return "{[key: string]: {%s}}" % ", ".join(buf)

def BuildTS(type, define):
    return "declare class %s %s" % (type, define)

def WriteTS(data, output):
    with codecs.open(output, 'w', "utf-8") as f:
        f.write(data)

#   win10 控制台调起一个py文件非常慢，不知原因，故此这个启动程序一块写在这里
OUTPUT_PATH = u"C:\\Users\\Administrator\\Desktop\\config\\export"
INPUT_PATH = u"C:\\Users\\Administrator\\Desktop\\config\\xlsx"
TS_PATH = u"C:\\Users\\Administrator\\Desktop\\config\\export\\type.d.ts"
    ...
    # try:
        ...
        for dirpath, _, filenames in os.walk(INPUT_PATH):
            for file in filenames:
                ...
                parsers = Write(OUTPUT_PATH, fullpath)
                ts.append(BuildTS(BuildName(file), BuildBody(parsers)))
        WriteTS("\n".join(ts), TS_PATH)
